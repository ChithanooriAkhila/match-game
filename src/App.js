...
class App extends Component {
  ...
  componentWillUnmount() {
    ...
    this.setState({matchRunning: false})
  }
  ...
  checkMatch = id => {
    const {imageToBeMatchedIndex} = this.state
    const randomValue = Math.floor(Math.random() * 20)
    if (id === imagesList[imageToBeMatchedIndex].id) {
      this.setState(prev => ({
        score: prev.score + 1,
        imageToBeMatchedIndex: randomValue,
      }))
    } else {
      this.endMatch()
    }
  ...
  render() {
    const {
      activeId,
      imageToBeMatchedIndex,
      seconds,
      score,
      matchRunning,
    } = this.state
    const {imageUrl} = imagesList[imageToBeMatchedIndex]
    const categoryThumbnails = imagesList.filter(
      image => image.category === activeId,
    )
    return (
        ...
        {matchRunning ? (
          <div className="match-game-container">
            ...
            <ul className="tabs-container">
              {tabsList.map(tab => (
                <TabItem
                  tabDetails={tab}
                  key={tab.tabId}
                  changeThumbnails={this.changeThumbnails}
                  activeId={activeId}
                />
              ))}
            </ul>
            ...
            <ul className="thumbnails-container">
              {categoryThumbnails.map(thumbnail => (
                <ThumbnailItem
